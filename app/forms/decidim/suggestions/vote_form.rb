...
module Decidim
  module Suggestions
    # A form object used to collect the data for a new suggestion.
    class VoteForm < Form
      include TranslatableAttributes
      include Virtus::Multiparams

      mimic :suggestions_vote
      ...
      def hash_id
        Digest::MD5.hexdigest(
          "#{suggestion_id}-#{document_number || author_id}-#{Rails.application.secrets.secret_key_base}"
        )
      end
      ...
      def suggestion_type
        @suggestion_type ||= suggestion&.scoped_type&.type
      end
      ...
      def document_number_authorized
        return if suggestion.document_number_authorization_handler.blank?
      ...
      def document_number_uniqueness
        errors.add(:document_number, :taken) if suggestion.votes.where(hash_id: hash_id).exists?
      end
      ...
      def handler_name
        suggestion.document_number_authorization_handler
      end
