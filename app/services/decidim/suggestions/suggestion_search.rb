...
module Decidim
  module Suggestions
    # Service that encapsulates all logic related to filtering suggestions.
    class SuggestionSearch < Searchlight::Search
      ...
      def base_query
        Decidim::Suggestion
          ...
          .joins("JOIN decidim_users ON decidim_users.id = decidim_suggestions.decidim_author_id")
      ...
      def search_search_text
        query
          ...
          .or(
            query.where(
              "cast(decidim_suggestions.id as text) ILIKE ?", "%#{search_text}%"
            )
          )
          .or(
            query.where(
      ...
      def search_type_id
        ...
        types = SuggestionsTypeScope.where(decidim_suggestions_types_id: type_ids).pluck(:id)
      ...
      def search_scope_id
        ...
        conditions = []
        conditions << "decidim_suggestions_type_scopes.decidim_scopes_id IS NULL" if clean_scope_ids.delete("global")
