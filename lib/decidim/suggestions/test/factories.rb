...
FactoryBot.define do
  factory :suggestions_type, class: "Decidim::SuggestionsType" do
  ...
  factory :suggestions_type_scope, class: "Decidim::SuggestionsTypeScope" do
    type { create(:suggestions_type) }
    scope { create(:scope, organization: type.organization) }
    supports_required { 1000 }

    trait :with_user_extra_fields_collection do
      type { create(:suggestions_type, :with_user_extra_fields_collection) }
    end
  end

  factory :suggestion, class: "Decidim::Suggestion" do
    title { generate_localized_title }
    description { Decidim::Faker::Localized.wrapped("<p>", "</p>") { generate_localized_title } }
    organization
    ...
    scoped_type do
      create(:suggestions_type_scope,
             type: create(:suggestions_type, organization: organization, signature_type: signature_type))
    end

    after(:create) do |suggestion|
      if suggestion.author.is_a?(Decidim::User) && Decidim::Authorization.where(user: suggestion.author).where.not(granted_at: nil).none?
        create(:authorization, user: suggestion.author, granted_at: Time.now.utc)
      end
      create_list(:suggestions_committee_member, 3, suggestion: suggestion)
    end
    ...
    trait :acceptable do
      ...
      after(:build) do |suggestion|
        suggestion.suggestion_votes_count = suggestion.scoped_type.supports_required + 1
      end
    end
    ...
    trait :with_user_extra_fields_collection do
      scoped_type do
        create(:suggestions_type_scope,
               type: create(:suggestions_type, :with_user_extra_fields_collection, organization: organization))
      end
    end
  end

  factory :suggestion_user_vote, class: "Decidim::SuggestionsVote" do
    suggestion { create(:suggestion) }
    author { create(:user, :confirmed, organization: suggestion.organization) }
  end

  factory :organization_user_vote, class: "Decidim::SuggestionsVote" do
    suggestion { create(:suggestion) }
    author { create(:user, :confirmed, organization: suggestion.organization) }
  ...
  factory :suggestions_committee_member, class: "Decidim::SuggestionsCommitteeMember" do
    suggestion { create(:suggestion) }
    user { create(:user, :confirmed, organization: suggestion.organization) }
    state { "accepted" }
